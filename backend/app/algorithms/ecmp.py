    ...
    def run(network: NetworkInput, config: AlgorithmConfig) -> SimulationResult:
        ...
        for demand in network.demands:
            ...
            try:
                ...
                all_paths = sorted(nx.all_shortest_paths(graph, demand.source, demand.target, weight="weight"))
            ...
            trace_events.append(SimulationTraceEvent(
                ...
                pathColor="#7c3aed",
                costCalculation="\n".join(calculations),
                metadata={"pathIds": path_ids},
            ))
            step += 1

            # ── Traffic distribution stage — the ONLY stage this PR changes.
